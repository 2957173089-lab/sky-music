...
interface Store {
  ...
  currentView: 'home' | 'search' | 'liked' | 'playlists' | 'settings';
  searchQuery: string;
  loading: boolean;
...
export const useStore = create<Store>((set, get) => ({
  ...
  nextSong: () => {
    const { queue, currentSong, recommendedSongs } = get();
    const all = [...queue, ...recommendedSongs.filter(r => !queue.find(q => q.id === r.id))];
    if (!all.length) return;
    const idx = all.findIndex(s => s.id === currentSong?.id);
    const next = all[(idx + 1) % all.length];
    if (next && next.id !== currentSong?.id) get().playSong(next, all);
    else if (all.length > 1) get().playSong(all[0], all);
  },
  ...
  prevSong: () => {
    ...
    if (prev) { skipCount = 0; get().playSong(prev); }
  },

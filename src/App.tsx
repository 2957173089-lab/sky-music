...
import {
  ...
  Plus, ChevronRight, Disc3, Zap, FolderPlus,
} from 'lucide-react';
...
// ─── Mobile Nav ───
function MobileNav() {
  const { currentView, setView, apiStatus, likedSongs } = useStore();
  const items = [
    { id: 'home' as const, icon: Home, label: '发现' },
    { id: 'search' as const, icon: Search, label: '搜索' },
    { id: 'liked' as const, icon: Heart, label: '喜欢', badge: likedSongs.length || undefined },
    { id: 'playlists' as const, icon: FolderPlus, label: '歌单' },
    { id: 'settings' as const, icon: Settings, label: '设置', dot: apiStatus === 'error' },
  ];
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 glass-strong flex z-30 safe-bottom rounded-t-2xl shadow-lg shadow-blue-100/30">
      {items.map(item => (
        <button key={item.id} onClick={() => setView(item.id)}
          className={`flex-1 flex flex-col items-center py-2 pt-2.5 transition relative ${
            currentView === item.id ? 'text-blue-500' : 'text-slate-400'
          }`}>
          {currentView === item.id && <div className="absolute top-0 left-1/2 -translate-x-1/2 w-6 h-0.5 bg-blue-500 rounded-full" />}
          <div className="relative">
            <item.icon size={19} />
            {item.badge && item.badge > 0 && (
              <span className="absolute -top-1 -right-2.5 bg-red-400 text-white text-[7px] rounded-full w-3 h-3 flex items-center justify-center font-bold">
                {item.badge > 9 ? '9+' : item.badge}
              </span>
            )}
            {'dot' in item && item.dot && <span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-red-400 rounded-full animate-pulse" />}
          </div>
...
export default function App() {
  const { currentView, initAudio, initApi, currentSong } = useStore();
  useEffect(() => { initAudio(); initApi(); }, [initAudio, initApi]);
  const renderPage = () => {
    switch (currentView) {
  ...
  return (
    <div className="h-full bg-gradient-to-br from-blue-50 via-sky-50 to-indigo-50 text-slate-800">
      <div className="flex h-full">
        <Sidebar />
        <main className={`flex-1 overflow-y-auto custom-scrollbar ${hasPlayer ? 'pb-[130px] md:pb-[84px]' : 'pb-[60px] md:pb-0'}`}>
          {renderPage()}
        </main>
      </div>
      <PlayerBar />
